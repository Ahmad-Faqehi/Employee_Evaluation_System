                            ...
                            <div class="card-body" style="direction: ltr;">
                                ...
                                if (isset($_POST['submit'])){
                                    ...
                                    if(empty($email) || empty($name) || empty($password) || empty($re_password)){
                                        ...
                                        $stmtz = $conn->prepare("INSERT INTO `admin`(`name`, `email`, `password`) VALUES (:name,:email,:pass) ");
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered text-center" id="dataTable" width="100%" cellspacing="0" dir="rtl">
                                        ...
                                        <tbody>
                                        <?php
                                        // TODO:: Chenge tavke name to admin
                                        $stmt=$conn->prepare("SELECT name,email,id FROM `admin` ");
                                        $stmt->execute();
                                        $count = 0;
                                        if($stmt->rowCount() > 0) :
                                            $rows = $stmt->fetchAll();
                                            foreach ($rows as $row) :
                                                ...
                                                <tr>
                                                    ...
                                                    <td><a href="edit-user.php?id=<?=$row['id']?>&role" class="btn btn-primary">تعديل</a> &nbsp; <a href="#dlete" onclick="myf(<?=$row['id']?>)" class="btn btn-danger">حذف</a> </td>
                                                </tr>
                                            <?php
                                            endforeach;
                                        endif;
...
<script>
    ...
    function  myf(id){
        ...
        $("#delete-link").attr("href", "delete-page.php?type=admin&id="+id);
