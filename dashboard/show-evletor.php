                    ...
                    <div class="col-xl-12 col-lg-12">
                        ...
                        <div class="card shadow mb-4">
                            ...
                            <div class="card-header py-3 ">
                                <h5 class="m-0 font-weight-bold text-dark text-center">بيانات المقيمين</h5>
                            </div>
                            <!-- Card Body -->
                            <div class="card-body" style="direction: ltr;">
                                ...
                                if (isset($_POST['submit'])){
                                    ...
                                    $stmt=$conn->prepare("SELECT email FROM evaluator WHERE email=:email");
                                    ...
                                    if(empty($email) || empty($name) || empty($password) || empty($re_password)){
                                        ...
                                        $stmtz = $conn->prepare("INSERT INTO `evaluator`(`name`, `email`, `password`) VALUES (:name,:email,:pass) ");
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered text-center" id="dataTable" width="100%" cellspacing="0" dir="rtl">
                                        ...
                                        $stmt=$conn->prepare("SELECT name,email,id FROM `evaluator` ");
                                        $stmt->execute();
                                        $count = 0;
                                        if($stmt->rowCount() > 0) :
                                            $rows = $stmt->fetchAll();
                                            foreach ($rows as $row) :
                                                ...
                                                <tr>
                                                    ...
                                                    <td><a href="edit-evl.php?id=<?=$row['id']?>" class="btn btn-primary">تعديل</a> &nbsp; <a href="#dlete" onclick="myf(<?=$row['id']?>)" class="btn btn-danger">حذف</a> </td>
                                                </tr>
                                            <?php
                                            endforeach;
                                        endif;
                                ...
                                <div class="text-center">
                                    <a href="#" data-toggle="modal" data-target="#add-user-form" class="btn btn-success btn-icon-split"><span class="icon text-white-50"><i class="fas fa-plus"></i></span><span class="text"> أضافة مقيم</span></a>
                                </div>
...
<div id="add-user-form" class="modal fade" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header bg-dark">
                <button type="button" class="close text-white" data-dismiss="modal">&times;</button>
                <h4 class="modal-title text-white"> أضافة مقيم جديد </h4>
            </div>
            <div class="modal-body" style="font-size: 16px;">
                <div class=" text-right p-3"  >
                    <form method="post" action="">
                        <div class="form-group">
                            <label class="text-dark">الأسم</label>
...
<script>
    ...
    function  myf(id){
        ...
        $("#delete-link").attr("href", "delete-page.php?type=evaluator&id="+id);
